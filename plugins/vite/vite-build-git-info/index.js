import { formatDate, execShellCommand } from './utils';
 ...
 * @param {String} key
 */
function getGitValueByKey(list = [], key) {
  const index = list.findIndex(item => item.includes(key));
  if (index > -1) {
    const gitStr = list[index].replace(key, '').trim();
    return {
      index,
      value: gitStr,
    };
...
function getGitCommitInfo() {
  return new Promise((resolve, reject) => {
    execShellCommand('git log -1').then((stdout, rej) => {
      ...
      // 提交作者
      const author = getGitValueByKey(stdoutArr, 'Author:')?.value;
      // 提交日期
      const { value: date, index } = getGitValueByKey(stdoutArr, 'Date:');
      // 提交信息
      const message = stdoutArr[index + 1];
      resolve({
        commit,
        author,
...
async function generateLog(config) {
  ...
  try {
    ...
    const hasDiff = await execShellCommand(`git diff ${branch} ${gitOriginName}/${branch}`);
    const gitLogInfo = {
